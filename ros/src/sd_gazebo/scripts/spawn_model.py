...
def usage():
    ...
    -px <x in meters> - optional: name of paramter holding initial pose, use 0 if left out
    -py <y in meters> - optional: name of paramter holding initial pose, use 0 if left out
    -pz <z in meters> - optional: name of paramter holding initial pose, use 0 if left out
    -pR <roll in radians> - optional: name of paramter holding initial pose, use 0 if left out
    -pP <pitch in radians> - optional: name of paramter holding initial pose, use 0 if left out
    -pY <yaw in radians> - optional: name of paramter holding initial pose, use 0 if left out
...
class SpawnModel():
    ...
    def parseUserInputs(self):
        ...
        for i in range(0,len(sys.argv)):
          ...
          if sys.argv[i] == '-px':
            if len(sys.argv) > i+1:
              self.initial_xyz[0] = float(rospy.get_param(sys.argv[i+1]))
          if sys.argv[i] == '-py':
            if len(sys.argv) > i+1:
              self.initial_xyz[1] = float(rospy.get_param(sys.argv[i+1]))
          if sys.argv[i] == '-pz':
            if len(sys.argv) > i+1:
              self.initial_xyz[2] = float(rospy.get_param(sys.argv[i+1]))
          if sys.argv[i] == '-pR':
            if len(sys.argv) > i+1:
              self.initial_rpy[0] = float(rospy.get_param(sys.argv[i+1]))
          if sys.argv[i] == '-pP':
            if len(sys.argv) > i+1:
              self.initial_rpy[1] = float(rospy.get_param(sys.argv[i+1]))
          if sys.argv[i] == '-pY':
            if len(sys.argv) > i+1:
              self.initial_rpy[2] = float(rospy.get_param(sys.argv[i+1]))
